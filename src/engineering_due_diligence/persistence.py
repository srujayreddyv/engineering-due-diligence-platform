"""Concrete SQLite persistence for repository archived-status evidence."""
...
_NORMALIZATION_VERSION = "repository-archived-normalization.v1"
_COLLECTOR_NAME = "public-github-repository-metadata"
_SOURCE_SNAPSHOT_NAMESPACE = "github-source-snapshot.v1"
_EVIDENCE_NAMESPACE = "repository-archived-evidence.v1"
_SOURCE_SNAPSHOT_PREFIX = "github-source-snapshot-"
_EVIDENCE_PREFIX = "repository-archived-evidence-"
...
_EXPECTED_SCHEMA_SQL = dict(
    zip(_EXPECTED_COLUMNS, _SCHEMA_STATEMENTS)
)
...
def _ensure_schema(connection: sqlite3.Connection) -> None:
    try:
        ...
        for table_name, expected_columns in _EXPECTED_COLUMNS.items():
            ...
            expected_sql = _EXPECTED_SCHEMA_SQL[table_name]
            if (
...
def _deterministic_identifier(
    ...
    material = "\0".join(
        (
            namespace,
            assessment_id,
            EvidenceKind.REPOSITORY_ARCHIVED.value,
            collection_attempt_id,
        )
    ).encode("utf-8")
...
def _evidence_from_row(row: sqlite3.Row) -> EvidenceRecord:
    if row["normalization_version"] != _NORMALIZATION_VERSION:
    ...
    if outcome is EvidenceOutcome.AVAILABLE:
        ...
        if type(archived_value) is not int or archived_value not in (0, 1):
            raise ValueError("stored archived value is invalid")
        value: Optional[bool] = bool(archived_value)
    else:
        value = None
    return EvidenceRecord(
